...
def OptimizerStatistics(optimizer):
...
def print_model_statistics():
  ...
  for param in params:
    utils.print_out("  %s, %s, %s, %d" % (param.name, str(param.get_shape()),
...
def configure_learning_rate(num_samples_per_epoch, global_step):
  ...
  """
  # Note: when num_gpus is > 1, this will actually have each clone to go
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    ...
    for (grad, param) in grads_and_vars:
      ...
      with tf.control_dependencies([grad]), tf.device(param.device):
