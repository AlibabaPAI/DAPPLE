...
def _build_network_base(images,
  ...
  for cell_num in range(hparams.num_cells):
    ...
    if cell_num in reduction_indices:
      ...
      cell_outputs.append(net)
      tf.logging.info('Reduction cell shape atlayer{}: {}'.format(
          true_cell_num, net.shape))
      true_cell_num += 1
      stage_outputs.extend(reduction_cell.cut_outputs)
      reduction_cell.cut_outputs = []
    net = normal_cell(
        net,
        ...
        prev_layer=cell_outputs[-2],
        cell_num=true_cell_num)
    if len(normal_cell.cut_outputs) > 0:
      device_idx = 1
    else:
      device_idx = 0
    with tf.device(devices[device_idx]):
      stage_outputs.extend(normal_cell.cut_outputs)
      normal_cell.cut_outputs = []
    ...
    cell_outputs.append(net)
    tf.logging.info('Normalnet shape atlayer {}: {}'.format(
        true_cell_num, net.shape))
    true_cell_num += 1
  ...
  with tf.device(devices[1]):
    ...
    with tf.variable_scope('final_layer',
                           custom_getter=network_utils.bp16_getter, reuse=tf.AUTO_REUSE):
      net = tf.nn.relu(net)
  ...
  end_points['stage_outputs'] = stage_outputs
