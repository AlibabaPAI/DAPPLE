...
class BaseCell(object):
  ...
  def __init__(self, num_conv_filters, operations, used_hiddenstates,
               ...
               drop_path_burn_in_steps, dep_outputs=None):
    ...
    self.devices = cluster_utils.get_pipeline_devices(FLAGS.pipeline_device_num)
    self.cut_outputs = []
    self.device_idx = 0
    self.dep_outputs = dep_outputs
    self.cut_cell_name = ""
    if FLAGS.num_cells == 6:
      self.cut_cell_name = "cell_3"
    elif FLAGS.num_cells == 12:
      self.cut_cell_name = "cell_7"
  ...
  def _reduce_prev_layer(self, prev_layer, curr_layer):
    ...
    if curr_filter_shape != prev_filter_shape:
      ...
      dep = None
      if tf.get_variable_scope().name == self.cut_cell_name or \
           tf.get_variable_scope().name == "reduction_cell_1":
        print("prev_1x1 cut feature!")
        self.cut_outputs.append(prev_layer)
        self.device_idx = 1
        dep_outputs = self.dep_outputs
        if dep_outputs is not None and dep_outputs[self.device_idx] is not None:
          dep = dep_outputs[self.device_idx] \
              if isinstance(dep_outputs[self.device_idx], list) else [dep_outputs[self.device_idx]]
      # cut feature map
      with tf.control_dependencies(dep), tf.device(self.devices[self.device_idx]):
  ...
  def _cell_base(self, net, prev_layer):
    ...
    if tf.get_variable_scope().name == "reduction_cell_1":
      device_idx = 1
    elif tf.get_variable_scope().name == self.cut_cell_name:
      device_idx = 0
    else:
    ...
    dep = None
    if tf.get_variable_scope().name == self.cut_cell_name:
      print("1x1 cut feature!")
      self.cut_outputs.append(net)
      self.device_idx = 1
      dep_outputs = self.dep_outputs
      if dep_outputs is not None and dep_outputs[self.device_idx] is not None:
        dep = dep_outputs[self.device_idx] \
  ...
  def __call__(self, net, scope=None, filter_scaling=1, stride=1,
    ...
    i = 0
    with tf.variable_scope(scope, custom_getter=bp16_getter, reuse=tf.AUTO_REUSE):
      with tf.device(self.devices[self.device_idx]):
        net = self._cell_base(net, prev_layer)
      with tf.device(self.devices[self.device_idx]):
        for iteration in range(5):
          with tf.variable_scope('comb_iter_{}'.format(iteration), reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('cell_output', reuse=tf.AUTO_REUSE):
  ...
  def _combine_unused_states(self, net):
    ...
    for idx, used_h in enumerate(used_hiddenstates):
      ...
      if should_reduce:
        ...
        with tf.variable_scope('reduction_{}'.format(idx), reuse=tf.AUTO_REUSE):
