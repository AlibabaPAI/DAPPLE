...
def add_arguments(parser):
  ...
  ## Dapple argument
  parser.add_argument("--cross_pipeline", type="bool", nargs="?", const=True,
                      default=False,
                      help="cross pipeline")
  parser.add_argument("--pipeline_device_num", type=int, default=1, help="device num in one pipeline")
  parser.add_argument("--micro_batch_num", type=int, default=1, help="num of batches in one pipeline")
  parser.add_argument("--num_replica", type=int, default=1, help="num of replica")
  parser.add_argument("--task_index", type=int, default=1, help="worker or ps task index")
  parser.add_argument("--worker_hosts", type=str, default="", help="""\
      IP address and port list\
      """)
  parser.add_argument("--job_name", type=str, default="", help="""\
      worker | ps\
      """)
  parser.add_argument("--np", type=int, default=1, help="num of process")
  parser.add_argument("--dapple_test", type="bool", nargs="?", const=True,
                      default=True,
                      help="whether open dapple placement")
  parser.add_argument("--fake_io", type="bool", nargs="?", const=True,
                      default=False,
                      help="whether to fake io or not")
  parser.add_argument("--gnmt16", type="bool", nargs="?", const=True,
                      default=False,
                      help="whether to enable gnmt16(16encoder+16decoder), default is gnmnt8(8encoder+8decoder)")
...
def create_hparams(flags):
  ...
  return tf.contrib.training.HParams(
      ## Dapple arguements
      cross_pipeline=flags.cross_pipeline,
      pipeline_device_num=flags.pipeline_device_num,
      ...
      np=flags.np,
      dapple_test=flags.dapple_test,
      fake_io=flags.fake_io,
      gnmt16=flags.gnmt16,
...
def run_main(flags, default_hparams, train_fn, inference_fn, target_session=""):
  ...
  if flags.inference_input_file:
    ...
    if FLAGS.cross_pipeline:
      cluster_manager = cluster_utils.get_cluster_manager(FLAGS, config_proto=session_config)
      target_session = cluster_manager.get_target()
...
if __name__ == "__main__":
  ...
  ## Print DEBUG log from JIT PROFILING  
