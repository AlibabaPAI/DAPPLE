...
class BaseModel(object):
  ...
  def __init__(self,
    ...
    with tf.device(devices[0]):
      ...
      if not self.extract_encoder_layers:
        self._set_train_or_infer(res, reverse_target_vocab_table, hparams, scope)
  ...
  def _set_train_or_infer(self, res, reverse_target_vocab_table, hparams, scope=None):
      ...
      if DAPPLE_TEST:
        ...
        for i in xrange(slice_num - 1, 0, -1):
          ...
          with tf.device(devices[i]):
            y_grads = tf.gradients(ys=ys, xs=prev_y+vars_i, grad_ys=prev_grads,
        ...
        #with tf.device(devices[0]):
        grads_0 = tf.gradients(ys=ys, xs=remained_vars, grad_ys=prev_grads,
            colocate_gradients_with_ops=True)
        ...
        for j in xrange(1, micro_batch_num):
          ...
          res = self.build_graph(hparams, scope, dep_outputs=dep_outputs)
          losses.append(res[1])
          all_outputs.append(res[-1]) ### push this micro_batch's outputs of all stage
          ys = losses[j]
          prev_grads=None
          for i in xrange(slice_num - 1, 0, -1):
...
#####
    ...
    utils.print_out("Format: <name>, <shape>, <size(MB)>, <(soft) device placement>")
    param_total_size = 0.0
    for param in params:
      param_total_size += np.asarray(param.shape.as_list()[0:]).prod()*4.0/1000/1000
      utils.print_out("  %s, %s, %.2f, %s" % (param.name, str(param.get_shape()),
                                        np.asarray(param.shape.as_list()[0:]).prod()*4.0/1000/1000,
                                        param.op.device))
  ...
  def build_graph(self, hparams, scope=None, dep_outputs=None):
    ...
    with tf.variable_scope(scope or "dynamic_seq2seq", dtype=self.dtype,
      ...
      if hparams.language_model:  # no encoder for language modeling
        ...
        with tf.control_dependencies(dep1), tf.device(devices[0]):
      ...
      device_idx += 1 ## Stage2
      if dep_outputs is not None and dep_outputs[device_idx] is not None:
        dep2 = dep_outputs[device_idx] \
              if isinstance(dep_outputs[device_idx], list) else [dep_outputs[device_idx]]
      logits, decoder_cell_outputs, sample_id, final_context_state, outputs_1 = (
          self._build_decoder(encoder_outputs, None, hparams, dep1, dep2))
      stage_outputs.append(outputs_1.rnn_output)

      ## Loss
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        with tf.device(devices[1]), tf.control_dependencies(dep2):
          loss = self._compute_loss(logits, decoder_cell_outputs)
          if hparams.fake_io:
            ## NOTE: When fake_io is enabled, force loss to zeros to prevent expected
            ## training OVERFLOW
            loss = loss * 0.0
      else:
      ...
      return logits, loss, final_context_state, sample_id, stage_outputs
  ...
  def _build_decoder(self, encoder_outputs, encoder_state, hparams, dep1=None, dep2=None):
    ...
    with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE) as decoder_scope:
      ...
      decoder_initial_state = decoder_initial_state \
          if isinstance(decoder_initial_state, list) else [decoder_initial_state]
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        with tf.device(devices[0]), tf.control_dependencies(dep1):
          if self.time_major:
        ...
        with tf.device(devices[0]), tf.control_dependencies(dep1):
          ...
          my_decoder_1 = tf.contrib.seq2seq.BasicDecoder(
              cell[0],
              helper_1,
              decoder_initial_state[0],)
          # Dynamic decoding
          outputs_1, final_context_state_1, _ = tf.contrib.seq2seq.dynamic_decode(
              my_decoder_1,
        ...
        with tf.device(devices[1]), tf.control_dependencies(dep2):
          outputs_1_copy = tf.identity(outputs_1.rnn_output)
          helper_2 = tf.contrib.seq2seq.TrainingHelper(
            outputs_1_copy, iterator.target_sequence_length,
            time_major=self.time_major)
    ...
    return logits, decoder_cell_outputs, sample_id, final_context_state, outputs_1
