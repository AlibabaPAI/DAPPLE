  ...
  with graph.as_default(), tf.container(scope or "train"):
    ...
    with tf.device(devices[0]):
      iterator = iterator_utils.get_iterator(
          ...
          shard_index=jobid,
          num_parallel_calls=hparams.num_gpus*4,
...
def _single_cell_dapple(hparams, mode, layer_id, residual_connection=False,
                        device_str=None, residual_fn=None):
  utils.print_out("  cell %d" % layer_id, new_line=False)
  cell =  _single_cell(unit_type=hparams.unit_type, num_units=hparams.num_units,
                      forget_bias=hparams.forget_bias, dropout=hparams.dropout,
                      mode=mode, residual_connection=residual_connection,
                      device_str=device_str, residual_fn=residual_fn)
  utils.print_out("")
  return cell
...
def _cell_list(unit_type, num_units, num_layers, num_residual_layers,
  ...
  for i in range(num_layers):
    ...
    single_cell = single_cell_fn(
        ...
        device_str=get_device_str(base_gpu, num_gpus),
...
def gradient_clip(gradients, max_gradient_norm):
  ...
  clipped_gradients, gradient_norm = gradients, tf.constant([max_gradient_norm])
